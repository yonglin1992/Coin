...
/**
 * Save the app's own log to dump along with adb bug report
 */
public abstract class LogSaver {
    ...
    public static LogSaver newInstance() {
        final boolean persistent = CoinServices.get().getBoolean(
                CoinServicesKeys.PERSISTENT_LOGSAVER,
                CoinServicesKeys.PERSISTENT_LOGSAVER_DEFAULT);
        if (persistent) {
            final int setSize = CoinServices.get().getInt(
                    CoinServicesKeys.PERSISTENT_LOGSAVER_ROTATION_SET_SIZE,
                    CoinServicesKeys.PERSISTENT_LOGSAVER_ROTATION_SET_SIZE_DEFAULT);
            final int fileLimitBytes = CoinServices.get().getInt(
                    CoinServicesKeys.PERSISTENT_LOGSAVER_FILE_LIMIT_BYTES,
                    CoinServicesKeys.PERSISTENT_LOGSAVER_FILE_LIMIT_BYTES_DEFAULT);
            return new DiskLogSaver(setSize, fileLimitBytes);
        } else {
            final int size = CoinServices.get().getInt(
                    CoinServicesKeys.IN_MEMORY_LOGSAVER_RECORD_COUNT,
                    CoinServicesKeys.IN_MEMORY_LOGSAVER_RECORD_COUNT_DEFAULT);
    ...
    private static final class DiskLogSaver extends LogSaver {
        ...
        private void initDiskLog() {
            mDiskLogger = Logger.getLogger(LogUtil.COIN_TAG);
            ...
            try {
                ...
                Log.e(LogUtil.COIN_TAG, "LogSaver: fail to init disk logger", e);
                return;
            }
        ...
        public void dump(PrintWriter writer) {
            for (int i = mSetSize - 1; i >= 0; i--) {
                ...
                try {
                    ...
                    Log.w(LogUtil.COIN_TAG, "LogSaver: can not find log file " + logFilePath);
                } catch (IOException e) {
                    Log.w(LogUtil.COIN_TAG, "LogSaver: can not read log file", e);
                }
            }
        }
